				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-edit"></i> User Data
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?=ADMIN_BASE_URL?>user_data/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
                        ...
                        if (isset($query)) {
                        	foreach ($query->result() as $row):
                        		...
                        		if (!isset($return_page))
									$return_page = 0;
                        		$edit_url = ADMIN_BASE_URL . 'user_data/create/' . $row->id;
                        ?>
						<tr>
							<td>
								<?php echo $i;?>
...
$(document).ready(function(){
	$(document).on('click', '.delete_record', function(e){
		...
		swal({
			...
			if(e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL?>user_data/delete",
					data: {'id': id},
					async: false,
					success: function() {
						swal("Deleted!", "user_data has been deleted.", "success");
						location.reload();
					}
				});
				swal("Deleted!", "user_data has been deleted.", "success");
	...
	$(document).on("click", ".view_details", function(event){
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL?>user_data/detail",
			data: {'id': id},
			async: false,
    ...
    $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
		...
		$.ajax({
		type: 'POST',
			url: "<?= ADMIN_BASE_URL ?>user_data/change_status",
